  ...
  constructor() {
    const cronJobArgsSchema = z.object({
      ...
      at: z
        ...
        .optional()
        .describe(
          '指定触发时间点（type=at 时需要，ISO 字符串，例如 2026-03-18T12:34:56.000Z；到点执行一次后自动停用。注意：time_now 返回的 iso 字段可直接用于此字段）',
        ),
    });
      ...
      async ({
        ...
        switch (action) {
          case 'list': {
            ...
            const lines = jobs
              .map((j: any) => {
                return `id=${j.id} type=${j.type} enabled=${j.isEnabled} running=${j.running} cron=${j.cron ?? ''} everyMs=${j.everyMs ?? ''} at=${j.at instanceof Date ? `${j.at.toISOString()}（北京时间 ${toLocalStr(j.at)}）` : (j.at ?? '')} instruction=${j.instruction ?? ''}`;
              })
              .join('\n');
          ...
          case 'add': {
            ...
            if (type === 'at') {
              ...
              return `已新增定时任务：id=${(created as any).id} type=at at=${(created as any).at?.toISOString?.() ?? ''}（北京时间 ${toLocalStr((created as any).at)}） enabled=${(created as any).isEnabled}`;
            }
