      ...
      async ({
        ...
        switch (action) {
          case 'list': {
            ...
            const lines = jobs
              .map((j: any) => {
                const atDisplay = j.at instanceof Date
                  ? new Date(j.at.getTime() - j.at.getTimezoneOffset() * 60000).toISOString().replace('Z', '').slice(0, 19)
                  : j.at ?? '';
                return `id=${j.id} type=${j.type} enabled=${j.isEnabled} running=${j.running} cron=${j.cron ?? ''} everyMs=${j.everyMs ?? ''} at=${atDisplay} instruction=${j.instruction ?? ''}`;
              })
              .join('\n');
          ...
          case 'add': {
            ...
            if (type === 'at') {
              ...
              const atDisplay = (created as any).at instanceof Date
                ? new Date((created as any).at.getTime() - (created as any).at.getTimezoneOffset() * 60000).toISOString().replace('Z', '').slice(0, 19)
                : (created as any).at?.toISOString?.() ?? '';
              return `已新增定时任务：id=${(created as any).id} type=at at=${atDisplay} enabled=${(created as any).isEnabled}`;
            }
